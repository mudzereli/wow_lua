...
--- ========== CONVERTED ACTION LIST ==========
  ...
  local function Simcraft_Standard()
    -- remorseless_winter
    ...
    if S.HowlingBlast:IsCastable(30, true) 
  ...
  local function Simcraft_BoS_Pooling()
  ...
  local function Simcraft_BoS_Ticking()
    ...
    -- if S.ArcaneTorrent:IsCastable() and HR.CDsON() and (Player:RunicPowerDeficit() > 20) then
    --   return ""
    -- end
  ...
  local function Simcraft_Obliteration()
  ...
  local function Simcraft_Cooldowns()
    ...
    -- 84cd8535-d709-4c1d-bcc2-0686790c68b0
    if HR.CDsON()
      and Settings.DeathKnight.Commons.UsePotions 
      and I.BattlePotionOfStrength:IsReady() 
      and Player:Buff(S.PillarOfFrost) 
      and Player:Buff(S.EmpowerRuneWeapon) then
      
      return "potion [84cd8535-d709-4c1d-bcc2-0686790c68b0]"
    end
    ...
    if talent_enabled("Cold Heart")
      ...
      if S.ChainsOfIce:IsCastable(30) 
  ...
  function FrostDK_APL()
    ...
    if Everyone.TargetIsValid() then
      ...
      ShouldReturn = Simcraft_Cooldowns();
    ...
    if HR.CDsON() 
      ...
      and S.BreathofSindragosa:CooldownRemains() < 5 then
        ShouldReturn = Simcraft_BoS_Pooling();
        if ShouldReturn then return ShouldReturn; end
        return "bos_pooling"
    end
    ...
    if Player:Buff(S.BreathofSindragosa) then
        ShouldReturn = Simcraft_BoS_Ticking();
        if ShouldReturn then return ShouldReturn; end
        return "bos_ticking"
    end

    --actions+=/run_action_list,name=obliteration,if=buff.pillar_of_frost.up&talent.obliteration.enabled
    if (Player:BuffP(S.PillarOfFrost) and talent_enabled("Obliteration")) then
        ShouldReturn = Simcraft_Obliteration();
        if ShouldReturn then return ShouldReturn; end
    end

    --actions+=/run_action_list,name=aoe,if=active_enemies>=2
    if HR.AoEON() and Cache.EnemiesCount[10] >= 2 then
      ShouldReturn = Simcraft_AOE();
      if ShouldReturn then return ShouldReturn; end
    end

    --actions+=/call_action_list,name=standard
    ShouldReturn = Simcraft_Standard();
    if ShouldReturn then return ShouldReturn; end
